...
if __name__ == "__main__":
  ...
  Faces = CreateFaces(size=cubesize,pixelWidth=screen_width/2)
  ...
  while running:
    ...
    if pygame.key.get_pressed()[pygame.K_COMMA] and not comma_pressed:
      if cubesize>1:
        cubesize, Faces, comma_pressed = cubesize-1, CreateFaces(cubesize-1), True
        for face in Faces:
          face[0] = RotateShape(face[0], "z", 145)
          face[0] = RotateShape(face[0], "y", -45)
          face[0] = RotateShape(face[0], "x", 45)
    ...
    if pygame.key.get_pressed()[pygame.K_PERIOD] and not period_pressed:
      cubesize, Faces, period_pressed = cubesize+1, CreateFaces(cubesize+1), True
      for face in Faces:
        face[0] = RotateShape(face[0], "z", 145)
        face[0] = RotateShape(face[0], "y", -45)
        face[0] = RotateShape(face[0], "x", 45)
